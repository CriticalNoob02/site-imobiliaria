...
export default function MenuToogle({title,link,uuids}:MenuToogleProps) {
    ...
    return (
        <div className="w-full h-full flex flex-row-reverse pr-10 items-center select-none">
            <div className=" ml-10">
                <div>
                    <label className="cursor-pointer">
                        ...
                        <svg viewBox="0 0 32 32" className={`h-12 transition-transform duration-500 ease-in-out ${menuStats ? "" : "rotate-45"}`}>
                ...
                <div className={`${menuStats ? "hidden opacity-0" : "visible opacity-100"} absolute right-14`}>
                    <NavItens 
                    data={{
                        titles: title,
                        links: link,
            ...
            <div>
                <button className="p-2 font-semibold transition text-slate-100 rounded-lg bg-lime-600 hover:bg-lime-500 hover:shadow-lg hover:shadow-lime-600/50 hover:-translate-y-0.5 ">
                    <a href="https://api.whatsapp.com/send?phone=5548991539040&text=Ol%C3%A1,%20estou%20visitando%20o%20site,%20gostaria%20de%20saber%20mais%20informa%C3%A7%C3%B5es" target="_blank">
                        Contato
                    </a>
                </button>
            </div>
